...
export function DmArea({ dmId }: Props) {
  const { messages, loading, subscribeToDm, sendMessage, deleteMessage, editMessage } = useDmStore();
  ...
  }, [targetUserId, targetUser, fetchUser]);
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col">
        ...
        {!loading && messages.length === 0 && (
          <div className="mt-auto mb-4">
            <div className="w-16 h-16 rounded-full bg-[#404249] flex items-center justify-center text-white mb-4 overflow-hidden">
              {targetUser?.avatar ? <img src={targetUser.avatar} className="w-full h-full object-cover" /> : <Hash size={40} />}
            </div>
            <h1 className="text-3xl font-bold text-black dark:text-white mb-2">{displayName}</h1>
            <p className="text-zinc-500 dark:text-zinc-400">This is the beginning of your direct message history with {displayName}.</p>
          </div>
        )}
